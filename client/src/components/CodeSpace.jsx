...
function CodeSpace() {
  ...
  const handleRun = async () => {
    ...
    try {
      setIsLoading(true);
      const { data: resultId } = await axios.post("/run", apiReqData);

      // console.log(typeof resultId);
      // console.log(resultId);

      let intervalId = setInterval(async () => {
        let { data: dataRes } = await axios.get("/results/" + resultId);
        ...
        if (status === "Done") {
          ...
          setIsLoading(false);
          if (jobOutput.stderr !== "") setState("output", jobOutput.stderr);
          else setState("output", jobOutput.stdout);
        } else {
    ...
    <div className="code-space">
      <div className="code-editor">
        <div className="file-options">
          <Dropdown
            onRequestClose={() => console.log("Clicked outside dropdown")}
          />
        </div>
        <Editor
          height="95%"
